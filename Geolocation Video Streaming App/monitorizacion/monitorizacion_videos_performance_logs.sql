...
CREATE TABLE `performance_logs` (
  ...
  `video_external_id` varchar(255) DEFAULT NULL,
  `event_type` varchar(50) DEFAULT NULL,
  `timestamp` datetime NOT NULL,
  `duration` int DEFAULT NULL,
  `device_info` varchar(255) DEFAULT NULL,
  `network_speed` varchar(100) DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_perf_user` (`user_id`),
  KEY `idx_perf_event` (`event_type`),
  CONSTRAINT `performance_logs_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
